...
const get = async (endpoint, id) => {
    const response = await axios.get(`http://localhost:3002/${endpoint}/${id}`);
    return response.data;
}
...
const updateData = async (endpoint, id, data) => {
    const response = await axios.put(`http://localhost:3002/${endpoint}/${id}`, data);
    return response.data;
}

const removeData =  async (endpoint, id) => {
    const response = await axios.delete(`http://localhost:3002/${endpoint}/${id}`);
    return response.data;
}
